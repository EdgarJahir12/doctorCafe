...
export class DrawingManagerManager {
  ...
  updateDrawingMode(drawingManager): Promise<void> {
    let value = (drawingManager.drawingMode != null) ? (drawingManager.drawingMode.figure != null ? drawingManager.drawingMode.figure : drawingManager.drawingMode) : null;
    return this._drawingManagers.get(drawingManager).then((dm: DrawingManager) => dm.setOptions({drawingMode: value}));
  }
